...
# This is steps/nnet3/align_lats.sh with minor changes:
#  - uses L_disambig.fst instead of L.fst
#  - and (to match decode) max-active=7000, lattice-beam=8, 
#    write compact lats with post-decode-acwt=10
# 2019  Joachim Fainberg    University of Edinburgh
...
if [ $stage -le 0 ]; then
  ...
  $cmd JOB=1:$nj $dir/log/compile_graphs.JOB.log \
    ...
    $scale_opts \
    $dir/tree $srcdir/${iter}.mdl  $lang/L_disambig.fst "$tra" \
...
if [ $stage -le 1 ]; then
  ...
  $cmd JOB=1:$nj $dir/log/generate_lattices.JOB.log \
    ...
    "$feats" "ark:| lattice-scale --acoustic-scale=$post_decode_acwt --write-compact=true ark:- ark:- | gzip -c >$dir/lat.JOB.gz" || exit 1;
fi
