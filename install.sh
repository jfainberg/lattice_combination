...
if [ $# -ne 1 ]; then
  ...
  echo "This script installs lattice-combine-light"
  echo "and fsts-compose binaries to Kaldi's source."
  exit 1
fi

KALDI_ROOT=$1

cp lattice-combine-light.cc fsts-compose.cc $KALDI_ROOT/src/latbin/ || exit 1

# Only amend if not already present
if ! grep -q " lattice-combine-light " $KALDI_ROOT/src/latbin/Makefile; then
    sed -i.bak "s:BINFILES =:BINFILES = lattice-combine-light fsts-compose:g" $KALDI_ROOT/src/latbin/Makefile || exit 1
fi

make -C $KALDI_ROOT/src/latbin || exit 1

echo "$0: Finished installing lattice-combine-light and fsts-compose"
